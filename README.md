...
在 `CameraMatch` 侧边栏的最上方有一个 **Match Background Resolution (匹配背景图分辨率)** 按钮：

1. 先给当前激活的相机添加背景图（相机属性 → Background Images）。
2. 点击该按钮，渲染分辨率 (`Resolution X/Y`) 与分辨率百分比会被自动设置为背景图的原生像素尺寸。
3. 如果当前相机没有添加任何背景图，插件会弹出提示（同时状态栏会显示警告信息）。
...
> **注意：地平线偏移 (Horizon Offset) 属于"强制俯仰覆盖"**。拖拽手柄产生的偏移会
> 一直保留到下一次编辑透视线段（编辑线段时偏移会被自动清零），解算结果会尊重该偏移。

👉 **拖拽地平线把手**（需要保持绘制模式）： 
1. 观察视口，在地平线的中央你会看到一个稍大一点的 **菱形手柄**。
2. 在绘制模式中，将鼠标按住该手柄**上下拖动**，插件就能强行扭转覆盖相机的俯仰角 (Pitch)。退出绘制模式后，参考线和地平线叠加层会被关闭。
3. 拖拽过程中按住 **`Shift`** 可进行精细微调（切换微调时不会丢失已拖拽的量）。
4. **注意：** 每当你之后再度尝试去改动或绘制核心的透视线段时，之前手动拖拉产生的地平线偏移量将会被立刻重置清零。保证最严谨的数学解算永远是以透视线为尊。
...
- **尺度锚点 = 3D 游标**：线条只能确定方向（消失点），无法确定场景尺度。解算会保持
  相机到 3D 游标的距离不变，把游标作为尺度与位置的锚点——因此匹配前请把 3D 游标
  放到照片中一个明确的参考点上。
